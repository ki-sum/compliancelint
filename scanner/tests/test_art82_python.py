"""Art. 82 Compliant AI systems which present a risk tests — obligation mapping.
...
Obligation mapping:
  has_corrective_action_procedure → ART82-OBL-2
"""
...
class TestArt82Obl2:
    ...
    def test_has_corrective_action_true_gives_partial(self, art82_module, tmp_path):
        """has_corrective_action_procedure=True → ART82-OBL-2 PARTIAL."""
        BaseArticleModule.set_context(_full_true_ctx())
        result = art82_module.scan(str(tmp_path))
        obl = _find(result, "ART82-OBL-2")
        assert len(obl) > 0
        assert obl[0].level == ComplianceLevel.PARTIAL

    def test_has_corrective_action_false_gives_non_compliant(self, art82_module, tmp_path):
        """has_corrective_action_procedure=False → ART82-OBL-2 NON_COMPLIANT."""
    ...
    def test_has_corrective_action_none_gives_utd(self, art82_module, tmp_path):
        """has_corrective_action_procedure=None → ART82-OBL-2 UNABLE_TO_DETERMINE."""
...
class TestArt82Structural:

    def test_all_1_obligation_ids_in_json(self, art82_module):
        """Obligation JSON must have exactly 1 obligation."""
        data = art82_module._load_obligations()
        obligations = data.get("obligations", [])
        assert len(obligations) == 1

    def test_obligation_coverage_present(self, art82_module, tmp_path):
    ...
    def test_no_answers_all_automatable_obligations_utd(self, art82_module, tmp_path):
        """When AI provides no answers, automatable obligations → UNABLE_TO_DETERMINE."""
        BaseArticleModule.set_context(_empty_ctx())
        result = art82_module.scan(str(tmp_path))
        automatable_ids = ["ART82-OBL-2"]
    ...
    def test_description_has_no_legal_citation_prefix(self, art82_module, tmp_path):
        """Finding descriptions must not start with legal citation prefix like [Art. 82(2)]."""
        BaseArticleModule.set_context(_full_true_ctx())
        result = art82_module.scan(str(tmp_path))
    ...
    def test_all_true_no_non_compliant(self, art82_module, tmp_path):
    ...
    def test_all_false_has_non_compliant(self, art82_module, tmp_path):
    ...
    def test_all_obligation_findings_present(self, art82_module, tmp_path):
        """All 1 obligation must appear in findings."""
        BaseArticleModule.set_context(_full_true_ctx())
        result = art82_module.scan(str(tmp_path))
        found_ids = {f.obligation_id for f in result.findings}
        expected_ids = {"ART82-OBL-2"}
