...
def cl_scan(
    ...
    # Single article → return full findings + sync hint
    if len(article_numbers) == 1:
        ...
        if config.saas_api_key and config.auto_sync:
            try:
                ...
                pass
        elif not config.saas_api_key:
            output += (
                "\n\n📊 Want to track compliance over time?\n"
                "   Run: cl_connect() — opens browser to create your free dashboard.\n"
                "   Then: cl_sync() to upload results.\n"
                "   Your code never leaves your machine — only findings are synced."
            )
        return output
    ...
    # Post-scan: auto-sync or hint (same logic as cl_scan_all)
    config = ProjectConfig.load(project_path)
    if config.saas_api_key and config.auto_sync:
        try:
            sync_result = cl_sync(project_path)
...
def cl_scan_all(project_path: str, project_context: str = "", ai_provider: str = "") -> str:
    ...
    # Phase 1: Art.5 (prohibitions) — no prerequisites
    # Phase 2: Art.6 (risk classification) — determines Art.9-15 applicability
    # Phase 3: Art.50 (transparency) — no prerequisites
    # Phase 4: Art.9-15 (high-risk only) — requires Art.6 = high-risk
    _PHASE_ORDER = [
        [5],           # Phase 1: prohibitions
        [6],           # Phase 2: risk classification
        [50],          # Phase 3: general transparency
        [9, 10, 11, 12, 13, 14, 15],  # Phase 4: high-risk obligations
    ]
...
@mcp.tool()
def cl_version() -> str:
    """Return ComplianceLint scanner version."""
    return json.dumps({"version": CL_VERSION, "tools": 24})
