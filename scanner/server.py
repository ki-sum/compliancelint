...
def cl_verify_evidence(project_path: str) -> str:
    ...
    summary["verification_instructions"] = (
        ...
        "§AT.19 Phase 4 (2026-05-13) — INCONSISTENCY CHECK (load-bearing): "
        "For each evidence item, cross-check the user's description / note "
        "AGAINST the resulting finding status. If the description contains "
        "language suggesting INCOMPLETENESS (e.g., 'Part 2 to follow', "
        "'second batch coming later', 'TODO: upload remaining files', "
        ...
        "entire articles. The dashboard cannot detect this; only AI "
        "natural-language judgement can. Inconsistencies do NOT change "
        "the status automatically — they appear as a separate "
        "'inconsistencies' section in your output so the user decides."
    )
